...
use crate::rpc::coordinator::{
    ConnectErrorRequest, HeartBeatRequest, JobDoneRequest, JobFailureRequest, JobRequestReply,
    JobRequestRequest, RegisterRequest,
};
use anyhow::Result;
...
async fn send_connect_error(
    ...
    client
        .connect_error(ConnectErrorRequest {
            id: id,
            worker_id: worker_id,
            ...
            if inner.valid == 1 && inner.job_type == 0 {
                ...
                for w in workers {
                    ...
                    match worker_connect {
                        Ok(mut worker) => {
                            ...
                            let rep_connect = worker
                                .map_data_request(MapDataRequestRequest {
                            ...
                            match rep_connect {
                                Ok(rep) => {
                                    ...
                                    loop {
                                        ...
                                        let l =
                                            kv_g.entry(k_unwrapped.clone()).or_insert(Vec::new());
                ...
                if c_error != 1 {
                    ...
                    for (key, values) in kv_g {
                        let output_r = (application.reduce_fn)(
                            key.clone(),
...
impl worker_server::Worker for Worker {
    async fn map_data_request(
        &self,
        request: Request<MapDataRequestRequest>,
    ) -> std::result::Result<Response<MapDataRequestReply>, Status> {
        ...
        if bucket == None {
            return Ok(Response::new(MapDataRequestReply {
                data: bytes::Bytes::new().to_vec(),
            }));
        }
